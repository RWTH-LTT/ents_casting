"""Module to fit long-term AR(2) models for the GBMPredictionModel of the time series parameters."""

import json
from pathlib import Path
import numpy as np
from sklearn.metrics import mean_absolute_error, mean_squared_error

from ents_casting.config import config
from ents_casting.forecast_models.lightgbm_model import LgbmForecastModel
from ents_casting.forecast_models.demand import DemandModel
from ents_casting.forecast_models.price_el import PriceElModel
from ents_casting.forecast_models.emission_el import EmissionElModel
from ents_casting.forecast_models.pv_local import PVlocalModel
from ents_casting.ar_models.long_term_error import LongTermArFitter


def main() -> None:
    """
    Main function to fit long term residual error models for all time series parameters.
    """
    for param in ["pv_local", "demand_el", "demand_heat", "demand_cold", "price_el", "emission_el"]:
        if param not in config["time_series_parameters"]:
            print(f"Skipping model training of {param}.")
            continue

        print(f"Evaluating long-term accuracy of {param} prediction model.")
        training_year = config["training_years"][param]
        hist_values, hist_forecasts, hist_forecast_errors = compute_long_term_residual_errors(
            param=param, year=training_year
        )
        compute_error_metrics(hist_values, hist_forecasts, hist_forecast_errors)

        print(f"Fitting AR(2) model for {param} forecast errors.")
        ar_model_fitter = LongTermArFitter(param=param, n_bins=10)
        bin_edges, hist_errors_by_forecast_bin, ar_params = ar_model_fitter.fit_long_term_AR(
            hist_values, hist_forecasts, hist_forecast_errors
        )
        ar_model_fitter.save_parameters(bin_edges, hist_errors_by_forecast_bin, ar_params)


def instantiate_model(param: str) -> LgbmForecastModel:
    """Instantiate the appropriate GBMPredictionModel based on the time series parameter."""
    if param == "pv_local":
        return PVlocalModel()
    elif param in ["demand_el", "demand_heat", "demand_cold"]:
        return DemandModel(ts_param=param)
    elif param == "price_el":
        return PriceElModel()
    elif param == "emission_el":
        return EmissionElModel()
    else:
        raise ValueError(f"Unknown parameter: {param}")


def compute_long_term_residual_errors(param: str, year: int = 2023) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Compute the residual errors for the given time series parameter. The residual error is the
    difference between the historical observations and forecasts made with forecast models that
    have perfect foresight on the future weather parameter.
    Args:
        param (str): The time series parameter to process.
        year (int): The year of which to compute the residual errors.

    Returns:
        np.ndarray: Historical values. Shape (n_forecast_dates * n_forecast_hours,).
        np.ndarray: Historical forecasts. Shape (n_forecast_dates, n_forecast_hours).
        np.ndarray: Historical forecast errors. Shape (n_forecast_dates, n_forecast_hours).
    """
    # Instantiate the forecast model
    model = instantiate_model(param)

    # Load data for prediction model
    Y_data, X_data = model.load_data(year)

    # Cutoff the first 168 hours due to lag features
    Y_data = Y_data[168:8760]
    for key in X_data:
        X_data[key] = X_data[key][168:8760]

    # Filter the parameters in X_data to only those used in the model
    X_data = {key: X_data[key] for key in model.input_parameters}

    # Convert to numpy arrays for easier indexing
    y = np.asarray(Y_data)

    # Iterate through the year in fold-sized steps and load the matching pretrained fold model
    hist_forecasts = np.zeros_like(y)
    n_days_per_fold = config["n_days_per_cross_validation_fold"]
    for prediction_interval_start in range(0, 8760, n_days_per_fold * 24):
        prediction_interval_end = min(prediction_interval_start + n_days_per_fold * 24, len(y))
        fold_idx = prediction_interval_start // (n_days_per_fold * 24)
        model.load_gbm_model(fold=fold_idx)

        # Predict for the current interval
        hist_forecasts[prediction_interval_start:prediction_interval_end] = model.predict_with_lag(
            {key: X_data[key][prediction_interval_start:prediction_interval_end] for key in X_data},
            lag_values=(
                list(y[prediction_interval_start - 168 : prediction_interval_start])
                if prediction_interval_start >= 168  # else use data from end of year
                else list(y[-(168 - prediction_interval_start) :])
            ),
        )

    # Ensure no negative values
    if param != "price_el":
        hist_forecasts = hist_forecasts.clip(min=0.0)

    hist_values = y
    hist_forecast_errors = hist_values - hist_forecasts

    return hist_values, hist_forecasts, hist_forecast_errors


def compute_error_metrics(
    hist_values: np.ndarray, hist_forecasts: np.ndarray, hist_forecast_errors: np.ndarray
) -> None:
    """
    Args:
        hist_values (np.ndarray): Historical values. Shape (n_forecast_hours).
        hist_forecasts (np.ndarray): Historical forecasts. Shape (n_forecast_hours).
        hist_forecast_errors (np.ndarray): Historical forecast errors. Shape (n_forecast_hours).

    """

    r2 = 1 - np.sum(hist_forecast_errors**2) / np.sum((hist_values - np.mean(hist_values)) ** 2)
    mae = mean_absolute_error(hist_values, hist_forecasts)
    rmse = np.sqrt(mean_squared_error(hist_values, hist_forecasts))

    print("Residual error metrics (long-term scenarios with perfect weather forecasts):")
    print(f"R-squared (R2): {r2:.4f}")
    print(f"Mean Absolute Error (MAE): {mae:.4f}")
    print(f"Root Mean Squared Error (RMSE): {rmse:.4f}")


if __name__ == "__main__":
    main()
