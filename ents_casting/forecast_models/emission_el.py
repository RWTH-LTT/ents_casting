...
def main(skip_hyper_parameter_tuning: bool = False, n_trials: int = 10) -> None:
    if "emission_el" not in config["time_series_parameters"]:
        print("Skipping model training of emission_el.")
        return
    training_year = config["training_years"]["emission_el"]
    model = EmissionElModel()

    if not skip_hyper_parameter_tuning:
        print("Tuning lightgbm hyperparameters for emission_el...")
        model.tune_hyperparameters(year=training_year, n_trials=n_trials)
    print("Training lightgbm models for emission_el...")
    model.cross_validate_and_save_models(year=training_year)


class EmissionElModel(LgbmForecastModel):
    def __init__(self) -> None:
        self.ts_param = "emission_el"
    ...
    def load_data(self, year: int) -> tuple[np.ndarray, Dict[str, np.ndarray]]:
        """
        Loads the data for training or testing the emission_el model.
        ...
        Y_data = complete_data["emission_el"][0:8760].astype(float)
